...
static byte controlRangeHour(GetDate getDate, byte _currentRelay, byte numberDataTime) {
  ...
  for (byte numberTime = 0; numberTime < 9; numberTime += 2  ) { //numberTime 0 2 4 6 8) {//провіряємо в секії всі години і для кожної ставимо флаг
    word tempGod_0 = dataAndTime[_currentRelay].godyna[numberTime + numberDataTime * 5];
    word tempGod_1 = dataAndTime[_currentRelay].godyna[numberTime + numberDataTime * 5 + 1];
    word tempMinute_0 = dataAndTime[_currentRelay].minut[numberTime + numberDataTime * 5];
    word tempMinute_1 = dataAndTime[_currentRelay].minut[numberTime + numberDataTime * 5 + 1];
    ...
    {
//      digitalWrite(releControl[_currentRelay].numberPin, ChangeHIGH );
      SubTimersFlagSections[numberTime / 2] = true;
    } else {
//      digitalWrite(releControl[_currentRelay].numberPin, ChangeLow);
  ...
  //   1 2 3 4 5
  //getDate.weekDay
  //dataAndTime[_currentRelay].dey[]
  boolean flagDey;
  int tempweekDayNumber = getDate.weekDayNumber - 1;

  if (tempweekDayNumber == -1)tempweekDayNumber = 6;

  if (dataAndTime[_currentRelay].dey[tempweekDayNumber + numberDataTime / 2 * 7] == 1) {
    flagDey = true;
